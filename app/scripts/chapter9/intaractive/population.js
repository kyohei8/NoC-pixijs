 ...
 */
class Population{
  constructor(renderer, mutationRate, num){
    ...
    // DNAの集団
    this.population = _.range(num).map((i) => {
      return new Face(renderer, new DNA(), 130 * (i + 1), 100);
    });
  }

  display(){
    this.population.forEach((f) => f.display());
  }
  ...
  reproduction(){
    ...
    this.population = this.population.map((r) => {
      ...
      return new Face(r.renderer, child, r.x, r.y);
    });
    // console.log(this.population[0].dna.genes[450].x);
    this.generation++;
  }
